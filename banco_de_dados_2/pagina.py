...
'''
Classe página que representa todos os níveis não folha da arvore, aqui são definidas todas as operações básicas
realizadas, as e operações auxiliares para controle e balanceamento
'''

class Pagina(object):
    '''
    Variaveis proximo e anterior são usadas para representar os ponteiros
    Variaveis de filhos e chaves fazem um link para uma lista, também se comportando como ponteiro para esses itens
    '''
    ...
    '''
    Operação de inserir, que procura o lugar exato para inserir os valores na arvore e ainda se já está ou nao
    nas paginas folha
    '''
    ...
    '''
    Metodo auxiliar do inserir, que atualizar as chaves no nivel folha
    '''    
    ...
    '''
    Operação de dividir e balancear as paginas da arvore em caso de necessidade
    '''
    ...
    '''
    Função auxiliar para ajustar os ponteiros de paginas irmãs ao atualizar suas chaves
    '''
    ...
    '''
    Fazer a mescla (merge) de paginas na hora de atualizar ou mesclar paginas irmãs
    '''
...
class Folha(Pagina):
    ...
    def __init__(self, folha_anterior, folha_proximo, pai, grau):
        ...
        self.folha = True
    '''
    Função de inserção na lista de paginas folha
    '''
    ...
    '''
    Função que remove um valor da lista de paginas folha, fazendo as devidas atualizações de ponteiros
    '''
    def remover(self, value):
        ...
        if p.pai == None:
            ...
            return p.remover_ajuste()
    '''
    Função auxiliar para balanceamento para a função de inserir
    '''
    def fazer_balanceamento(self):
        index = (self.grau+1)//2
        r = self.chaves[index-1]
