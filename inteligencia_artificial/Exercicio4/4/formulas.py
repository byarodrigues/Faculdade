...
class Formula:
    ...
    def _to_set(cls, formula):
        ...
        while True:
            ...
            subformula = set(cls._to_flat(formula, OR_OP, False))
            if subformula not in new_formula:
                new_formula.append(subformula)
            # Return new formula
            return new_formula
        ...
        if isinstance(formula, list):
            ...
            for pos, subformula in enumerate(formula):
                if isinstance(subformula, list):
                    change, nova_subformula = cls._cnf_change(formula)
                    if change:
    ...
    def _cnf(cls, formula):
        ...
        while True:
            ...
            LOG.debug('...Converting to CNF: %s', formula)
            if not change:
                return formula
    ...
    def evaluate(self, values):
        ...
        LOG.debug('...Evaluating formula with values: %s', formula)
